...
function replaceChars(text) { //text is the html object of the input box - not the value of the box.
  let replaced = text.value; //instance the text parameter for easier manipulation
  for (symbol of symbols) { //iterate through every symbol
    for (proc of symbol.proc) { //iterate through each proc of each symbol
      let searchMask = proc;
...
let submitSymbols = function() { //called on <input> keydown
  ...
  for (i in symbols) { //iterate through passed symbols
    for (altSymbol of symbolAlts) {//iterate through each symbol alt
      if (
  ...
  for (equation of equations) { // iterate through each equation
    let curScore = 0; //score of current equation we are evaluating
    for (let i = 0; i < symbols.length; i++) { //iterate through each symbol given
      if (equation.symbols.indexOf(symbols[i].toLowerCase()) !== -1) { //if the symbol is found within the array of equation's symbols
        curScore++; //increment score
      }
    }
    let threshold = (document.getElementById("strictSwitch").checked) ? Math.floor(symbols.length/2) : 0; //if the strictSwitch is checked, an equation will only be matched if it has half of the symbols that are passed. if not checked, it must have at least one symbol
    if (curScore > threshold) { //push any equations found with the amount of symbols as let passed through by threshold
      found.push({
  ...
  for (foundEquation of found) { //for each equation we found to match
    if (addedToBody.indexOf(foundEquation) === -1) { //if the found equation hasn't already been put into the array
...
//TODO: optimize sort algorthim
  for (let j = 0; j < addedToBody.length; j++) { //run loop as many times as we have indexes
    for (let k = 0; k < addedToBody.length - 1; k++) { //iterate through each element of array
      if (addedToBody[k].score < addedToBody[k + 1].score) { //swap low values so the biggest come first
